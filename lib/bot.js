...
function bot(torrent, bot) {
  ...
  //bot.onText(/\/help/, async msg => {help((ctx) =>
    //ctx.reply(
      //`Hi ${ctx.chat.first_name} \nUse /start to get the list of commands.`
    //)
  //);
  ...
  bot.onText(/\/help/, async msg => {
	...
	const fromid = msg.from.id;
        bot.sendMessage(from, `Hi ${msg.from.first_name} \nUse /start to get the list of commands.`);
      } else {
        bot.sendMessage(msg.chat.id, unauthorizedMessage);
      }
  });
  ...
  bot.onText(downloadRegex, (msg, match) => {
    if(authorizedChats.includes(`${msg.chat.id}`)){
        ...
        const reply = async torr => {
          ...
          mess1 += `📊: ${torr.status} | ${torr.downloaded} of ${torr.total}\n\n`;
          //mess1 += `🗃Size: ${torr.total}\n\n`;
          if (!torr.done) {
            //mess1 += `📥Downloaded: ${torr.downloaded} |📈: (${torr.progress}%) |🏎: ${torr.speed}|\n\n`;
            //mess1 += `🏎Speed: ${torr.speed}\n\n`;
            mess1 += `📈: ${torr.progress}% |🏎: ${torr.speed}\n\n`;
	    mess1 += `SeedsTorr: ${torr.seeds}\n`;
	    mess1 += `SeedsTorren: ${torren.seeds}\n`;
	    mess1 += `SeedsTorrent: ${torrent.seeds}\n`;
	  ...
	  //const chatId = msg.chat.id;
	  //const user = bot.update.msg(chatId);
	  //const user1 = bot.update.msg(chat_id);
	  //mess1 += `${from, user}\n\n`;
	  //mess1 += `${from, user1}\n`;
	  mess1 += `🙇‍♂️Added by: ${msg.from.first_name} |@${msg.from.username}|\n\n`;
          //mess1 += `🧲Magnet URI:`;
          try {
            if (messageObj) {
    ...
    const onDriveUpload = (torr, url) => bot.sendMessage(from, `✅ ${torr.name} Your torrent has been uploaded to the GDrive\n${url}\n|@${msg.from.username}|`);
    const onDriveUploadStart = torr => bot.sendMessage(from, `Uploading ${torr.name} to GDrive`);

    if (link.indexOf("magnet:") !== 0) {
      bot.sendMessage(from, "That is not a magnet link");
    } else {
      //bot.sendMessage(from, "Starting download... ${msg.chat.first_name} | @${msg.chat.username}");
      bot.sendMessage(from, `Starting download... @${msg.from.username}`);
      try {
        ...
        bot.deleteMessage(from, onDriveUpload);
  ...
  bot.onText(statusRegex, (msg, match) => {
    if(authorizedChats.includes(`${msg.chat.id}`)){
        ...
        if (link.indexOf("magnet:") !== 0) {
          bot.sendMessage(from, `Bro! Magnet links ONLY! \n Try a Proper Request`);
        } else if (!torr) {
          bot.sendMessage(from, "Currently it's not downloading. Please add");
        } else {
          let mess1 = "";
          ...
          mess1 += `🗃Size: ${torr.total}\n`;
          if (!torr.done) {
            mess1 += `📥Downloaded: ${torr.downloaded} |📈:(${torr.progress}%)\n\n`;
            mess1 += `🏎Speed: ${torr.speed}\n`;
	    ...
	    //const chatId = msg.chat.id;
	    //const user= bot.getChat(chatId);
	    //mess1 += `${from, user}\n`;
            //mess1 += `Link: ${torr.downloadLink}\n\n`;
          }
	  mess1 += `🙇‍♂️ Added by: ${msg.from.first_name} |@${msg.from.username}|\n\n`;
  ...
  bot.onText(removeRegex, (msg, match) => {
    ...
    try {
      torrent.remove(link);
      bot.sendMessage(from, `Removed... 🗑 @${msg.from.username}`);
    } catch (e) {
